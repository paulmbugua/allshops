...
class _SupportScreenState extends ConsumerState<SupportScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    for (final faq in faqs) {
      categories
          .putIfAbsent(faq['category']?.toString() ?? 'Help', () => [])
          .add(faq);
    }
    return Scaffold(
      appBar: AppBar(
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(
                        colors: [Color(0xFF173B34), Color(0xFF2A6658)],
                      ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Guidance for ${_pretty(profile['roleCode']?.toString() ?? widget.membership.role)} on web and mobile.',
                        ...
                        (faq) => Card(
                          ...
                          child: ExpansionTile(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 8),
                              Text(
                                (faq['platforms'] as List<dynamic>? ?? const [])
                                    .join(' · '),
                                style: const TextStyle(
                                  fontSize: 10,
                                  fontWeight: FontWeight.w800,
                  ...
                  if (faqs.isEmpty)
                    const _Notice('No answers match that search.'),
                  const SizedBox(height: 18),
                  const Card(
                    child: ListTile(
                      leading: CircleAvatar(
                        backgroundColor: Color(0xFFFFE4DA),
                        child: Icon(
                      ...
                      subtitle: Text(
                        'Share the time, screen, branch, device and request or local reference. Never share passwords or card details.',
                      ),
                    ),
                  ),
                ],
...
class _DiagnosticCards extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final organization = data?['organization'] is Map
        ...
        : const <String, dynamic>{};
    final subscription = organization['subscription'] is Map
        ? Map<String, dynamic>.from(organization['subscription'] as Map)
        : const <String, dynamic>{};
    return Row(
      children: [
        Expanded(
          child: _DiagnosticCard(
            ...
            label: 'Subscription',
            value: subscription['status']?.toString() ?? 'Not configured',
            color: const Color(0xFF2B9A6D),
          ),
        ),
        ...
        Expanded(
          child: _DiagnosticCard(
            ...
            value:
                '${devices.where((item) => item['status'] == 'ACTIVE').length}',
            color: const Color(0xFF4979C7),
          ),
        ),
        const SizedBox(width: 9),
        Expanded(
          child: _DiagnosticCard(
            ...
            value: '$conflicts',
            color: conflicts == 0
                ? const Color(0xFF2B9A6D)
                : const Color(0xFFE59D24),
          ),
        ),
      ],
    );
